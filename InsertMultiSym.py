...
def create_template(json_data):
    ...
    for i in range(1, max_body_num + 1):
        bodies_wheels_code += insert_body(i)

    j = 0
    for wheel in json_obj["wheels"]:
        j += 1
        motors.append(f"motor{j}")
        for num in wheel["BodyNum"]:
            if num == '0':
                bodies_wheels_code += insert_wheel(j)
                if wheel["Symmetrical"] == "True":
                    motors.append(f'motor{j}SYM')
                    bodies_wheels_code += insert_wheel(f'{j}SYM') # naming needs to be matched in the controller, maybe import an array with all the names
            else:
